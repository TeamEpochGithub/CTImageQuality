...
def _BatchNorm(channels, eps=1e-3, momentum=0.01):
    ...
    return nn.InstanceNorm2d(channels, eps=eps, momentum=momentum)
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, param, num_classes=1000, stem_channels=32, feature_size=1280, drop_connect_rate=0.2):
        ...
        for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in self.config:
            ...
            blocks.append(
                MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
            for _ in range(repeats - 1):
                drop_rate = drop_connect_rate * (len(blocks) / total_blocks)
                blocks.append(
                    MBConvBlock(out_channels, out_channels, kernel_size, 1, expand_ratio, se_ratio, drop_rate))
...
class EfficientNet_v1(nn.Module):
    def __init__(self, input_dim, drop_connect_rate=0.25):
        ...
        block_length = 0
        blocks1 = []
        config1 = [self.config[0], self.config[1]]
        for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in config1:
            # drop connect rate based on block index
            drop_rate = drop_connect_rate * (block_length / total_blocks)
            blocks1.append(
                MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
            block_length += 1
            for _ in range(repeats - 1):
        ...
        blocks2 = []
        config2 = [self.config[2]]
        for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in config2:
            # drop connect rate based on block index
            drop_rate = drop_connect_rate * (block_length / total_blocks)
            blocks2.append(
                MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
        ...
        blocks3 = []
        config3 = [self.config[3], self.config[4]]
        for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in config3:
            # drop connect rate based on block index
            drop_rate = drop_connect_rate * (block_length / total_blocks)
            blocks3.append(
                MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
            ...
            for _ in range(repeats - 1):
                ...
                blocks3.append(
                    MBConvBlock(out_channels, out_channels, kernel_size, 1, expand_ratio, se_ratio, drop_rate))
                block_length += 1
        self.blocks3 = nn.Sequential(*blocks3)

        blocks4 = []
        config4 = [self.config[5], self.config[6]]
        for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in config4:
            # drop connect rate based on block index
            drop_rate = drop_connect_rate * (block_length / total_blocks)
            blocks4.append(
                MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
            ...
            for _ in range(repeats - 1):
                ...
                blocks4.append(
                    MBConvBlock(out_channels, out_channels, kernel_size, 1, expand_ratio, se_ratio, drop_rate))
                block_length += 1
        self.blocks4 = nn.Sequential(*blocks4)

        # blocks5 = []
        # config5 = [self.config[6]]
        # for in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, repeats in config5:
        #     # drop connect rate based on block index
        #     drop_rate = drop_connect_rate * (block_length / total_blocks)
        #     blocks5.append(
        #         MBConvBlock(in_channels, out_channels, kernel_size, stride, expand_ratio, se_ratio, drop_rate))
        ...
        #             MBConvBlock(out_channels, out_channels, kernel_size, 1, expand_ratio, se_ratio, drop_rate))
        #         block_length += 1
        # self.blocks5 = nn.Sequential(*blocks5)
...
#     net = EfficientNet_v1()
#     x_image = Variable(torch.randn(1, 3, 256, 256))
#     y = net(x_image)